...
def main():
    ...
    # )
    t5_text_embedder = T5TextEmbedder(
        pretrained_path="google/flan-t5-xl", max_length=128
    )
    ella = load_ella(args.pretrained_ella, device="cuda", dtype=torch.float16)
    ...
    t5_text_embedder.requires_grad_(False)
    ...
    if args.gradient_checkpointing:
        ella.enable_gradient_checkpointing()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for _step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(ella):
                ...
                encoder_hidden_states = ella(
                    t5_text_embedder(batch["captions"]), timesteps
                )
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        unwrapped_ella = unwrap_model(ella)
                        ella_state_dict = unwrapped_ella.state_dict()

                        torch.save(ella_state_dict, save_path)
